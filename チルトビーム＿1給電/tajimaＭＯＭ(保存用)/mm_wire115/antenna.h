...
void Calculation(void)
{
	...
	double r_pole1,r_pole2,r_pole3,r_pole5;						//ダイポールの長さ
	...
	r_pole5 = r_pole1_2/(2.0*M_PI) - r_pole1/(2.0*M_PI);
	...
	//--全セグメント数設定
	n_pole = int(2.0*(nbun_pole1+nbun_pole2+nbun_pole3+1+1+1+nbun_pole1_2+nbun_pole2_2+nbun_pole3_2+1+1+1) + (nbun_pole4*2+1+1) + 2.0*(nbun_pole5+1+1));
...
//------------------------------------------------オーバーラップ:内円から外円への線------------------------------------------------------------
	for(j = 0; j < 1; ++j){
		rrx = naihan * cos((2.0*M_PI/360)*(45) + (2.0*M_PI/(nbun_pole1+nbun_pole2))*(nbun_pole1-1+0));										//始点位置計算x
		rry = naihan * sin((2.0*M_PI/360)*(45) + (2.0*M_PI/(nbun_pole1+nbun_pole2))*(nbun_pole1-1+0));										//始点位置計算y
		rrz = r_pole3 ;	//始点位置計算z
		drx = naihan * cos((2.0*M_PI/360)*(45) + (2.0*M_PI/(nbun_pole1+nbun_pole2))*(nbun_pole1-1+1));										//終点位置計算x
		dry = naihan * sin((2.0*M_PI/360)*(45) + (2.0*M_PI/(nbun_pole1+nbun_pole2))*(nbun_pole1-1+1));										//終点位置計算y
		drz = r_pole3 ;		//終点位置計算z
		//--セグメント長 単位方向ベクトルの各成分の計算
		dcl = sqrt( pow( (rrx-drx),2 )
				  + pow( (rry-dry),2 )
...
//------------------------------------------------内円から外円への線------------------------------------------------------------
	...
	for(j = 0; j < nbun_pole5; ++j){
		//--始点 終点 セグメント長計算
		rrx = naihan + (j+0)*(r_pole5/nbun_pole5);										        //始点位置計算x
		rry = 0; 												//始点位置計算y
		rrz = r_pole3;		//始点位置計算z
		drx = naihan + (j+1)*(r_pole5/nbun_pole5); 												//終点位置計算x
		dry = 0;												//終点位置計算y
		drz = r_pole3;		//終点位置計算z
		//--セグメント長 単位方向ベクトルの各成分の計算
		dcl = sqrt( pow( (rrx-drx),2 )
...
//------------------------------------------------⑩反射板一給電------------------------------------------------------------
	...
	for(j = 0; j<nbun_pole1+nbun_pole2+nbun_pole3+1+1+1+nbun_pole1_2+nbun_pole2_2+nbun_pole3_2+1+1+1+nbun_pole5+1+1; ++j){
		//--計算結果代入
		RX[i] = RX[j];											//始点位置x
		RY[i] = RY[j];											//始点位置y
		RZ[i] = (-1.0)*RZ[j];									//始点位置z
...
//------------------------------------------------内円オーバーラップ:給電棒(-z)------------------------------------------------------------
	for(j = 0; j < 1; ++j){
		//--始点 終点 セグメント長計算
		rrx = naihan + (nbun_pole5/2-1)*(r_pole5/nbun_pole5);										        //始点位置計算x
		rry = 0; 												//始点位置計算y
		rrz = -r_pole3;		//始点位置計算z
		drx = naihan + (nbun_pole5/2+0)*(r_pole5/nbun_pole5); 												//終点位置計算x
		dry = 0;												//終点位置計算y
		drz = -r_pole3;		//終点位置計算z
		//--セグメント長 単位方向ベクトルの各成分の計算
		dcl = sqrt( pow( (rrx-drx),2 )
...
//------------------------------------------------内円給電棒------------------------------------------------------------
	...
	for(j = 0; j < nbun_pole4*2; ++j){
		//--始点 終点 セグメント長計算
		rrx = naihan + r_pole5/2;												//始点位置計算x
		rry = 0.0;											//始点位置計算y 半径
		rrz = (-1.0)*(r_pole3) + (j+0)*(r_pole3/nbun_pole4);		//始点位置計算z
		drx = naihan + r_pole5/2;												//終点位置計算x
		dry = 0.0;												//終点位置計算y 半径
...
//------------------------------------------------内円オーバーラップ:給電棒(+z)------------------------------------------------------------
	for(j = 0; j < 1; ++j){
		//--始点 終点 セグメント長計算
		rrx = naihan + (nbun_pole5/2+0)*(r_pole5/nbun_pole5);										        //始点位置計算x
		rry = 0; 												//始点位置計算y
		rrz = r_pole3;		//始点位置計算z
		drx = naihan + (nbun_pole5/2-1)*(r_pole5/nbun_pole5); 												//終点位置計算x
		dry = 0;												//終点位置計算y
		drz = r_pole3;		//終点位置計算z
		//--セグメント長 単位方向ベクトルの各成分の計算
		dcl = sqrt( pow( (rrx-drx),2 )
...
//-------------------------------------------------------------------------------------------------------------------
	...
	SEGN[10] = nbun_pole1_2 +1;
	SEGN[11] = nbun_pole2_2 +1;
	SEGN[12] = nbun_pole3_2 +1;

	SEGN[13] = nbun_pole5 +2;

	SEGN[14] = nbun_pole4*2 +2;
	...
	//					第１給電以外で制御を行う．
	//========================================================
	FEDP[0] = SEGN[0]+SEGN[1]+SEGN[2]+SEGN[3]+SEGN[4]+SEGN[5]+SEGN[6]+SEGN[7]+SEGN[8]+SEGN[9]+SEGN[10]+SEGN[11]+SEGN[12]+SEGN[13]-16+(SEGN[14]-2)/2-1+3;
